    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12">
        <router-link to="/products" v-for="(product, index) in products" :key="index" class="group cursor-pointer block">
          ...
          <h3 class="text-2xl md:text-3xl font-bold text-brand-dark text-center mb-3 group-hover:text-brand-gold transition-colors">{{ product.title }}</h3>
          <p class="text-gray-500 text-center text-lg">{{ product.desc }}</p>
        </router-link>
      </div>
